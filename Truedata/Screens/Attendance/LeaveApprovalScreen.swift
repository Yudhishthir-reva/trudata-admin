//
//  LeaveApprovalScreen.swift
//  Truedata
//

import SwiftUI

struct LeaveApprovalScreen: View {

    @Environment(\.dismiss) private var dismiss
    @StateObject private var viewModel = LeaveApprovalViewModel()
    @State private var pendingAction: LeaveStatusAction?
                ...
                LazyVStack(spacing: 12) {
                    ForEach(viewModel.filteredItems) { item in
                        LeaveApprovalRequestCard(
                            ...
                            onApprove: {
                                pendingAction = LeaveStatusAction(
                                    leaveId: item.id,
                                    staffId: item.userId,
...
private struct LeaveApprovalRequestCard: View {
    let item: LeaveTeamWiseItem
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                infoRow(label: "Duration:", value: item.durationLabel)
                infoRow(label: "Leave type:", value: item.leaveType.isEmpty ? "N/A" : item.leaveType)
                infoRow(label: "Applied On:", value: item.appliedDate.isEmpty ? "N/A" : item.appliedDate)
